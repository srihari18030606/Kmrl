...
def allocate_maintenance_bays(maintenance, all_trains_map):

    def priority_key(m):
        train_name = m["train"]
        train_obj = all_trains_map.get(train_name)

        sensor_priority = 1 if train_obj and train_obj.sensor_alert else 0
        job_card_priority = 1 if train_obj and train_obj.open_job_card else 0
        cleaning_priority = train_obj.days_since_cleaning if train_obj else 0
    ...
    sorted_maintenance = sorted(maintenance, key=priority_key)

    maintenance_bays = []
    maintenance_queue = []

    for idx, item in enumerate(sorted_maintenance):

        train_name = item["train"]
        train_obj = all_trains_map.get(train_name)

        # -------- BUILD EXPLANATION --------
        reasons = []

        if train_obj.sensor_alert:
            reasons.append("Sensor alert detected")

        if train_obj.open_job_card:
            reasons.append("Open job card pending")

        if train_obj.days_since_cleaning >= 10:
            reasons.append(f"High cleaning delay ({train_obj.days_since_cleaning} days)")

        if not reasons:
            reasons.append("General maintenance condition")

        priority_reason = ", ".join(reasons)

        if idx < MAINTENANCE_BAYS:

            maintenance_bays.append({
                "bay": idx + 1,
                "train": train_name,
                "maintenance_reason": item["why"],
                "placement_reason": f"Assigned to maintenance bay because of high priority: {priority_reason}."
            })
            ...
            maintenance_queue.append({
                "train": train_name,
                "maintenance_reason": item["why"],
                "placement_reason": f"Queued because higher-priority trains occupied available bays (only {MAINTENANCE_BAYS} bays available)."
            })

    return maintenance_bays, maintenance_queue
...
def evaluate_trains(trains, traffic_level=3):
    ...
    if not eligible and not maintenance:
        maintenance_bays, maintenance_queue = allocate_maintenance_bays(
            maintenance,
    ...
    # ---------------------------
    # NEW: MAINTENANCE BAY ALLOCATION
    # ---------------------------
    maintenance_bays, maintenance_queue = allocate_maintenance_bays(
        maintenance,
        all_trains_map
    ...
    for train in eligible:
        if train.is_branded and train.contract_days_remaining is not None and train.contract_total_exposure is not None:
            ...
            if train.contract_days_remaining > 0:
                ...
                time_pressure = 1 + (2 / (abs(train.contract_days_remaining) + 1))
    ...
    LAST_DEPOT_LAYOUT = {
        ...
        "track3": [t["train"] for t in track3],
        "track4": [t["train"] for t in track4],
        "inspection": [m["train"] for m in maintenance][:IBL_CAPACITY],
        "maintenance_bays": maintenance_bays,
        "maintenance_queue": maintenance_queue
    }
